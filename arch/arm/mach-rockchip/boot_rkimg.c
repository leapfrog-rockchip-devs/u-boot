...
struct blk_desc *rockchip_get_bootdev(void)
{
	...
	if (dev_type == IF_TYPE_MMC) {
		...
		printf("=====>emmc zhan pin start 08 60\n");
		writel(0x0, 0xff2bc008);
		writel(0x1f000800, 0xff2bc00c);
		writel(0x00f00060, 0xff2bc00c);
		writel(0x10000000, 0xff2bc004);
		writel(0x00070000, 0xff2bc00c);
		printf("=====>emmc zhan pin end\n");
		...
		snprintf(mmc_cid, sizeof(mmc_cid),
				"androidboot.cid=%x%x%x%x%x%x%x%x%x%x%x%x%x%x",
				(mmc->cid[0] >> 24) & 0xffff,
				(mmc->cid[0] >> 8) & 0xffff, mmc->cid[0] & 0xff,
				(mmc->cid[1] >> 24) & 0xffff,
				(mmc->cid[1] >> 8) & 0xffff, mmc->cid[1] & 0xff,
				(mmc->cid[2] >> 24) & 0xff, (mmc->cid[2] >> 16) & 0xff,
				(mmc->cid[2] >> 8) & 0xff, mmc->cid[2] & 0xff,
				(mmc->cid[3] >> 24) & 0xff, (mmc->cid[3] >> 16) & 0xff,
				(mmc->cid[3] >> 8) & 0xff, mmc->cid[3] & 0xff);
	}
#endif

	printf("PartType: %s, cid: %s\n", part_get_type(dev_desc), mmc_cid);
	env_update("bootargs", mmc_cid);
