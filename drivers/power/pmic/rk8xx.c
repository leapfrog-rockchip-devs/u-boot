...
static int rk8xx_shutdown(struct udevice *dev)
{
	...
	case RK817_ID:
		...
		dev_off = BIT(0);
	  //ebable charge
	 	ret =dm_i2c_read(dev,0xe6,&val,1);
		val|=0x40;
	  ret =dm_i2c_write(dev, 0xe6,&val,1);
		break;
	default:
...
int rk817_shutdown(struct udevice *dev)
{
	struct rk8xx_priv *priv = dev_get_priv(dev);
	u8 val, dev_off, devctrl_reg;
	int ret = 0;
	...
	case RK817_ID:
		...
		dev_off = BIT(0);
		// disable charge 
		ret =dm_i2c_read(dev,0xe4,&val,1);
		val&=0x7f;
	   ret =dm_i2c_write(dev, 0xe4,&val,1);
	 // ebable charge
	// 	ret =dm_i2c_read(dev,0xe6,&val,1);
		//val&=0xbf;
	//	val|=0x40;
