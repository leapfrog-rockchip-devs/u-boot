...
extern int rk817_shutdown(struct udevice *dev);
...
int charge_full_flag=1;
int charge_Nofull_flag=1;
int charge_low_power_flag=0;
int charge_online=0;
//#define ChargeOKPoweroff 
/*
...
static int charge_animation_ofdata_to_platdata(struct udevice *dev)
{
	...
	pdata->auto_off_screen_interval =
		dev_read_u32_default(dev, "rockchip,auto-off-screen-interval", 10);
...
//	int ret;
 // charge_online = fg_charger_get_chrg_online(dev);
	voltage = fuel_gauge_get_voltage(fg);
		printf("frank debug enter voltage=%d,charge_online=%d\n\n",voltage,charge_online);
	if (voltage < 0)
		return -EINVAL;
		
	while (voltage < pdata->low_power_voltage + 100) {
		mdelay(50);
		printf("frank debug enter 1111 voltage=%d\n\n",voltage);
		/* Check charger online */
		charging = fg_charger_get_chrg_online(dev);
			printf("%s: charging, online=%d\n\n",
			       __func__, charging);
		if (voltage <= 2700) {
			...
			//pmic_shutdown(pmic);
     	 rk817_shutdown(pmic);
			printf("Cpu should never reach here,charge shutdown failed !\n");
			continue;
		}
			if (charging <= 0) {
			printf("%s: Not charging, online=%d. Shutdown...\n",
	...
	if ((boot_mode != BOOT_MODE_CHARGING) &&
	    (boot_mode != BOOT_MODE_UNDEFINE)) {
	    	if(charge_low_power_flag==0)
	    	{
				...
				{
					printf("enter charge animation show\n");
				}
	}
#endif
...
show_images:
		...
		//static void change_Wakeuptime(void)
		current = fuel_gauge_get_current(fg);
		//printf("get current = %d\n", current);
		if((soc==100)&&(current<100))
		{
				charge_Nofull_flag=1;
				if(charge_full_flag==1)
				{
					printf("frank debug auto_wakeup_interval\n");
					charge_full_flag=0;
					pdata->auto_wakeup_interval=300;
					autowakeup_timer_init(dev, pdata->auto_wakeup_interval);
				}
		}
		else
		{
			charge_full_flag=1;
			if(charge_Nofull_flag==1)	
			{
				printf("frank debug auto_wakeup_interval2\n");
				charge_Nofull_flag=0;
				pdata->auto_wakeup_interval=35;
				autowakeup_timer_init(dev, pdata->auto_wakeup_interval);
			}		
		}
		#ifdef ChargeOKPoweroff
		if(soc==100)
		{
			 if(count>2)
			 {
