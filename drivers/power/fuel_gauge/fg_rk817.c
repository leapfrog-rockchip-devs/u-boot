...
#define BAT_EXS		0x00eb
#define BAT_CON			BIT(4)

#define CHRG_CUR_SEL	0x00E4
#define CHRG_EN	0x00E6
...
/*CHRG_CUR_SEL */
#define	CHRG_CUR_500MA	(0x07)
#define	CHRG_CUR_1000MA	(0x00)
#define	CHRG_CUR_1500MA	(0x01)
#define	CHRG_CUR_2000MA	(0x02)
#define	CHRG_CUR_3500MA	(0x06)
#define CHRG_VOL_4100MV		(0x00)
...
/* charger charge current set define */
enum charger_current_type {
	 Current_450MA=0,
	 Current_850MA,
};
...
struct rk817_battery_device {
	...
	int		ts2_vol_multi;
	u32 *ntc_from;
	int charge_current_set;
	int pre_charge_current_set;
	int charge_input_current_set;
	int pre_input_charge_current_set;
	int charge_enable_flag;
};
...
static void rk817_bat_init_ts_detect(struct rk817_battery_device *battery)
{
	...
	rk817_bat_write(battery, ADC_CONFIG0, value | 0x20);
	/* source current to TS pin */
	value = rk817_bat_read(battery,ADC_CONFIG1);
	rk817_bat_write(battery, ADC_CONFIG1, value & 0xf7);
	/* ts pin flow out current in active state */
	value = rk817_bat_read(battery,ADC_CONFIG1);
	value &= 0xcf;
	value |= 0x10;
	rk817_bat_write(battery, ADC_CONFIG1,value );//FLOW_OUT_40uA
	battery->ntc_factor = 21;
	battery->charge_input_current_set =Current_450MA;
	battery->pre_input_charge_current_set=Current_850MA;
	battery->pre_charge_current_set =CHRG_CUR_1000MA;
	battery->charge_current_set =CHRG_CUR_500MA;
}
...
static void rk817_bat_not_first_pwron(struct rk817_battery_device *battery)
{
	...
	if (battery->pwroff_min >= 0) {
	rk817_bat_init_coulomb_cap(battery, pre_cap);
		//rk817_bat_init_coulomb_cap(battery, pre_soc  * battery->fcc / 1000 / 100);
		//rk817_bat_init_coulomb_cap(battery, pre_soc  * battery->fcc / 1000 / 100 + 1);
		rk817_bat_get_capacity_mah(battery);
		is_charge = 0;
		DBG("frank debug MMMM battery->dsoc=%d,battery->rsoc=%d\n",battery->dsoc,battery->rsoc);
		goto finish;
	}
	if (now_cap == 0) {
...
static void rk817_bat_set_charge_current(struct rk817_battery_device *battery,
					int charge_current)
{
	u8 current_ctrl;

	current_ctrl = rk817_bat_read(battery, CHRG_CUR_SEL);
	current_ctrl &=0xf8 ;
	current_ctrl |= charge_current;
	rk817_bat_write(battery, CHRG_CUR_SEL, current_ctrl);
}
				      ...
				      int charger)
{
	...
	battery->charge_input_current_set =Current_850MA;
	battery->charge_enable_flag=1;
	/* charger changed */
	if (old_charger != charger) {
		if (charger == NO_CHARGER) {
			...
			printk("DC OR AC CHARGE\n");
			
			if(battery->temperature<10)
			{
				printk("DC OR AC CHARGEILIM_450MA\n");
				rk817_bat_set_input_current(battery, ILIM_1500MA);
					rk817_bat_set_charge_current(battery, CHRG_CUR_1000MA);
					battery->charge_input_current_set =Current_450MA;
					battery->charge_enable_flag =1;
			}
			else
			{
				printk("DC OR AC ILIM_850MA\n");
			  rk817_bat_set_input_current(battery, ILIM_1500MA);
			  rk817_bat_set_charge_current(battery, CHRG_CUR_1000MA);
			  battery->charge_input_current_set =Current_850MA;
			  battery->charge_enable_flag =1;
...
{
	return rk817_bat_read(battery, 0xe6)&0x40;
}
static void rk817_bat_enable_charge(struct rk817_battery_device *battery)
{
	int i;
	i=rk817_bat_read(battery, 0xe6);
	rk817_bat_write(battery, 0xe6, i|0x40);
}
...
static void rk817_bat_disable_charge(struct rk817_battery_device *battery)
{
	int i;
	i=rk817_bat_read(battery, 0xe6);
	rk817_bat_write(battery, 0xe6, i&0xbf);
}
...
static int rk817_bat_get_bat_ts(struct rk817_battery_device *battery)
{
	...
	temp_value = rk817_bat_read(battery, BAT_TS_H) << 8;
	temp_value |= rk817_bat_read(battery, BAT_TS_L);
...
//if (battery->temperature < 28) {
	...
	/*} else if(battery->temperature > 30){
			value = rk817_bat_read(battery,ADC_CONFIG1);
			value &= 0xcf;
			value |= 0x30;
		  rk817_bat_write(battery, ADC_CONFIG1,value );//FLOW_OUT_80uA
      battery->ntc_factor = 43;
      if(res>200)
      res=res-200;
	}*/
	printk("++++++++++++ bat_ts_vol: %d ntc_factor=%d,battery->temperature=%d,res=%d\n\n\n", bat_ts_vol,battery->ntc_factor,battery->temperature,res);
	return res;
}

static int rk817_bat_update_temperature(struct rk817_battery_device *di)
{
//	struct rk817_battery_device *battery = dev_get_priv(dev);
	...
	if (ntc_size) {
		res = rk817_bat_get_ntc_res(di);
		printf("bat ntc degree: R=%d\n", res);
		if (res == TS1_NOT_READY) {
			di->temperature = old_temperature;
			return TS1_NOT_READY;
		}
		if (res < ntc_table[ntc_size - 5]) {
			...
			if(is_bat_charge_en(di))
			{
				rk817_bat_disable_charge(di);
			} 
		...
		//	printf("bat ntc upper max degree:1111111 R=%d,temperature=%d,di->ntc_degree_from=%d\n\n\n\n", res,di->temperature,di->ntc_degree_from);
	}
		if ((di->temperature <= 0) || (di->temperature > 42)) 
		{
			printk("disable charge\n");
			if(is_bat_charge_en(di))
			{
				rk817_bat_disable_charge(di);
			} 
		...
		else 
		{
			...
			//	i = rk817_bat_read(di, 0xe6);
	        //    printf( "charge current  i =%x\n\n\n", i);
				printk("enable charge\n");
			if(!is_bat_charge_en(di))
			{
					rk817_bat_enable_charge(di);
			}	
			
			 di->chrg_type =rk817_bat_get_charger_type(di);
			 if (di->chrg_type == DC_CHARGER || di->chrg_type == AC_CHARGER)//dc 
			 {
						di->charge_input_current_set =Current_850MA;
				 		if(di->temperature <15)
						{
							printk("enable dc charge 500MA\n");
							di->charge_current_set =CHRG_CUR_500MA;
						}
						else
						{
	
						    	printk("enable dc charge 1000M Adi->charge_current_set=%d,di->charge_input_current_set=%d \n",di->charge_current_set,di->charge_input_current_set);
								di->charge_current_set =CHRG_CUR_1000MA;
						}						
			 }
			 else
			 {
				//	printk("enable usb charge1111 ILIM_450MA\n");//PC USB charge 
				//rk817_bat_set_input_current(di, ILIM_450MA);
					di->charge_input_current_set =Current_450MA;
				//	printk("frank debug di->charge_input_current_set=%d,di->pre_input_charge_current_set=%d\n\n",di->charge_input_current_set,di->pre_input_charge_current_set);
				//	return 0;
			 }
				
	    if(di->charge_current_set!=di->pre_charge_current_set)
		{
			di->pre_charge_current_set=di->charge_current_set;
			if(di->charge_current_set==CHRG_CUR_500MA)
			{
				//rk817_bat_set_charge_current(di, CHRG_CUR_500MA);
			}
			else
			{
				//rk817_bat_set_charge_current(di, CHRG_CUR_1000MA);
			}
		}
		
		if(di->charge_input_current_set!=di->pre_input_charge_current_set)
		{
			printk("set charge current\n\n");
			di->pre_input_charge_current_set=di->charge_input_current_set;
			/*if(di->charge_input_current_set==Current_450MA)
			{
				rk817_bat_set_input_current(di, ILIM_450MA);
			}
			else
			{
				rk817_bat_set_input_current(di, ILIM_850MA);
			}*/
		}
...
static int rk817_fg_ofdata_to_platdata(struct udevice *dev)
{
	...
	if (!prop) {
		...
		sign = degree_from[0];
		battery->ntc_degree_from = degree_from[1];
		if (sign)
			battery->ntc_degree_from = -battery->ntc_degree_from;

		battery->ntc_size = len / sizeof(u32);
	}

	if (battery->ntc_size) {
		battery->ntc_table = calloc(len, 1);
		if (!battery->ntc_table) {
			printf("calloc ocv_table fail\n");
			return -ENOMEM;
		}

		ret = dev_read_u32_array(dev, "ntc_table",
					 battery->ntc_table, battery->ntc_size);
		if (ret < 0) {
	...
	//	charge_online=rk817_bat_dwc_otg_check_dpdm(battery);
	printk("temperature1111=%d,charge_online=%d\n", battery->temperature,charge_online);
	DBG("OCV Value:");
...
static int rk817_fg_init(struct rk817_battery_device *battery)
{
	...
	if (battery->variant == RK817_ID)
		rk817_bat_charger_setting(battery, battery->chrg_type);
	value = rk817_bat_read(battery, 0xe6);
	value&=0x7b;
//value&=0x7f;
	rk817_bat_write(battery, 0xe6, value |0x01);
